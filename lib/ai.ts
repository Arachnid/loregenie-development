...
export async function aiGenerate<T>(
  ...
  const fullPrompt = PROMPT_TEMPLATE(
    kind,
    prompt,
    context.map((c) => formatObject(c)),
    formatObject(template),
  );

  // Preparing messages array
  const messages: Array<ChatCompletionMessageParam> = messageHistory
    ? [...messageHistory]
    : [];

  // Check if a similar system message already exists in message history
  const hasSimilarSystemMessage = messages.some(
    (message) => message.role === "system" && message.content === fullPrompt,
  );
...
export async function modifyResponse<T>(
  prompt: string,
